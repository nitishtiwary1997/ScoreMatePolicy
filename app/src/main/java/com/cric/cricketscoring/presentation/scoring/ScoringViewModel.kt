...
import com.cric.cricketscoring.data.remote.FirebaseMatchDataSource
...
import com.cric.cricketscoring.domain.usecase.tournament.CalculatePointsTableUseCase
...
class ScoringViewModel @Inject constructor(
    ...
    fun recordBall(
        ...
        viewModelScope.launch {
            ...
            // Immediately clear the out player from the crease
            if (isWicket) {
            ...
            if (isWicket) {
                ...
                dialog = ScoringDialog.SelectBowler(availableBowlers)
            }
    ...
    fun onSelectBatsman(playerId: String) {
        viewModelScope.launch {
            ...
            // Deterministically select the remaining player at the crease based on who got out/retired.
            // If the striker was dismissed, the remaining player is the non-striker.
            // If the non-striker was dismissed, the remaining player is the striker.
            val remainingPlayer = if (dialog.dismissedIsStriker) {
                if (currentOffStrike.isNotEmpty()) currentOffStrike else currentOnStrike
            } else {
                if (currentOnStrike.isNotEmpty()) currentOnStrike else currentOffStrike
            }

            val (newOnId, newOffId) = if (dialog.overCompleteAfter) {
                remainingPlayer to playerId
            } else {
                if (dialog.dismissedIsStriker) {
                    playerId to remainingPlayer
                } else {
                    remainingPlayer to playerId
                }
            }
    ...
    fun undoLastBall() {
        viewModelScope.launch {
            ...
            val innings = match.currentInnings()
            val currentBalls = state.currentBalls
            val lastBall = currentBalls.lastOrNull() ?: return@launch

            val currentOn = if (innings == 1) match.innings1OnStrikeId else match.innings2OnStrikeId
            val currentOff = if (innings == 1) match.innings1OffStrikeId else match.innings2OffStrikeId

            var prevOn = currentOn
            var prevOff = currentOff

            if (lastBall.isWicket && lastBall.wicketType != WicketType.RETIRED_HURT) {
                val dismissedId = lastBall.dismissedPlayerId ?: lastBall.batsmanId
                
                val newBatsmanId = if (currentOn.isNotEmpty() && currentOn != dismissedId && currentOn != lastBall.batsmanId) currentOn
                                   else if (currentOff.isNotEmpty() && currentOff != dismissedId && currentOff != lastBall.batsmanId) currentOff
                                   else ""
                
                val remainingId = if (currentOn.isNotEmpty() && currentOn != newBatsmanId) currentOn else currentOff

                if (dismissedId == lastBall.batsmanId) {
                    prevOn = dismissedId
                    prevOff = remainingId
                } else {
                    prevOn = lastBall.batsmanId
                    prevOff = dismissedId
                }
            } else {
                val totalBallRuns = lastBall.runs + lastBall.extras
            ...
            val restoredMatch = updateBatsBowl(match, innings, prevOn, prevOff, lastBall.bowlerId)
            repository.updateMatch(restoredMatch)
            repository.undoLastBall(matchId, innings)
        }
    }
    ...
    private fun startLiveSyncFromFirestore(ownerId: String, matchId: String) {
    ...
    /** End the match early — works in any innings. Result is declared based on current score. */
    fun endMatchEarly() {
        viewModelScope.launch {
            val state = _uiState.value
            val match = state.match ?: return@launch
            ...
            val innings = match.currentInnings()
            val innings1Total = state.innings1Score?.totalRuns ?: 0
            val innings2Runs  = state.innings2Score?.totalRuns ?: 0
            val innings2Wickets = state.innings2Score?.wickets ?: 0
            val innings1BattingTeamName = match.battingTeamName(1)
            val innings2BattingTeamName = match.battingTeamName(2)
            val battingTeamNum = match.battingTeamNumber(2)
            val battingTeamSize = state.players.count { it.team == battingTeamNum }

            val result: String = when {
                innings == 1 -> "Match Abandoned — No Result"
                innings2Runs > innings1Total -> {
                    val wicketsInHand = battingTeamSize - 1 - innings2Wickets
                    "$innings2BattingTeamName won by $wicketsInHand wicket${if (wicketsInHand == 1) "" else "s"}"
                }
                innings2Runs < innings1Total -> {
                    val margin = innings1Total - innings2Runs
            ...
            val updatedMatch = match.copy(
                ...
                innings2Completed = innings == 2
            )
            repository.updateMatch(updatedMatch)
            _uiState.update { it.copy(dialog = ScoringDialog.MatchComplete(result)) }

            // Update tournament fixture if applicable (only when 2nd innings ended)
            if (match.fixtureId != null && match.tournamentId != null && innings == 2) {
                val targetReached = innings2Runs > innings1Total
                completeTournamentFixture(
                    match = match,
                    result = result,
                    targetReached = targetReached,
            ...
            // Remove live score doc
            viewModelScope.launch(Dispatchers.IO) { liveScoreDataSource.clear(matchId) }
        }
    }

    fun transferScoringPermission(inviteeMobile: String, role: String) {
        val match = _uiState.value.match ?: return
        val ownerId = match.ownerId.ifBlank { userSession.userId }
        viewModelScope.launch(Dispatchers.IO) {
            matchDataSource.transferScoringPermission(
                ownerId = ownerId,
                matchId = match.id,
            ...
            if (role == "Editor") {
                val inviteeQuery = com.google.firebase.firestore.FirebaseFirestore.getInstance()
                    .collection("users")
                ...
                if (inviteeId.isNotEmpty()) {
                    val updatedMatch = match.copy(currentEditorId = inviteeId)
                    repository.insertMatchLocally(updatedMatch)
                }
            }
        }
    }
