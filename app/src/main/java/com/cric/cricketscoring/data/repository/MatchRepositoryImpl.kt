package com.cric.cricketscoring.data.repository
...
class MatchRepositoryImpl @Inject constructor(
    ...
    override suspend fun createMatch(match: Match, players: List<Player>) {
        val ownerId = userSession.userId.ifBlank { "anonymous" }
        val enrichedMatch = match.copy(ownerId = ownerId, currentEditorId = ownerId)
        matchDao.insertMatch(enrichedMatch.toEntity())                    // isSynced = false
        playerDao.insertPlayers(players.map { it.toEntity() })   // isSynced = false
        syncScope.launch {
            try {
                firebase.saveMatch(ownerId, enrichedMatch)
                matchDao.markMatchSynced(enrichedMatch.id)
            } catch (_: Exception) { }
            try {
                firebase.savePlayers(ownerId, enrichedMatch.id, players)
                playerDao.markPlayersSynced(enrichedMatch.id)
            } catch (_: Exception) { }
        }
    }
    ...
    override suspend fun updateMatch(match: Match) {
        ...
        syncScope.launch {
            try {
                firebase.saveMatch(ownerId, match)
    ...
    override suspend fun recordBall(ball: Ball) {
        val match = matchDao.getMatchByIdSync(ball.matchId)
        val ownerId = match?.ownerId?.ifBlank { userSession.userId } ?: userSession.userId
        ballDao.insertBall(ball.toEntity())          // isSynced = false
        syncScope.launch {
            try {
                firebase.saveBall(ownerId, ball)
    ...
    override suspend fun undoLastBall(matchId: String, innings: Int) {
        val match = matchDao.getMatchByIdSync(matchId)
        val ownerId = match?.ownerId?.ifBlank { userSession.userId } ?: userSession.userId
        val lastBallId = ballDao.getLastBallId(matchId, innings)
        ballDao.deleteLastBall(matchId, innings)
        lastBallId?.let { id ->
            syncScope.launch { runCatching { firebase.deleteLastBall(ownerId, matchId, innings, id) } }
        }
    }
    ...
    private suspend fun syncPendingToCloud() {
        ...
        try {
            ...
            orphanPlayers.groupBy { it.matchId }.forEach { (matchId, players) ->
                try {
                    firebase.savePlayers(uid, matchId, players.map { it.toDomain() })
    ...
    private suspend fun syncMatchBundle(entity: MatchEntity) {
        ...
        val ownerId = entity.ownerId.ifBlank { userSession.userId }
        try {
            firebase.saveMatch(ownerId, entity.toDomain())

            val players = playerDao.getPlayersForMatchSync(matchId)
            if (players.isNotEmpty()) {
                firebase.savePlayers(ownerId, matchId, players.map { it.toDomain() })
            }

            val balls = ballDao.getBallsForMatchSync(matchId)
            for (ball in balls) {
                firebase.saveBall(ownerId, ball.toDomain())
            }
    ...
    override suspend fun insertMatchLocally(match: Match) {
        matchDao.insertMatch(match.toEntity().copy(isSynced = true))
    }

    override suspend fun insertPlayersLocally(players: List<Player>) {
        playerDao.insertPlayers(players.map { it.toEntity().copy(isSynced = true) })
    }

    override suspend fun syncBallsLocally(matchId: String, innings: Int, balls: List<Ball>) {
        ballDao.deleteBallsForInnings(matchId, innings)
        ballDao.insertBalls(balls.map { it.toEntity().copy(isSynced = true) })
    }

    override suspend fun checkForAssignedMatches() {
        val phoneNumber = userSession.phoneNumber
        if (phoneNumber.isBlank()) return
        try {
            val assignedMatches = firebase.detectAssignedMatches(phoneNumber)
            for (match in assignedMatches) {
                insertMatchLocally(match)
            }
        } catch (_: Exception) {}
    }
    ...
    override suspend fun deleteMatch(matchId: String) {
        ...
        syncScope.launch {
            try {
                firebase.deleteMatch(ownerId, matchId)
            } catch (_: Exception) {}
        }
    }
}
